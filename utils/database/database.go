...
// Connect initializes MongoDB. An empty URI keeps the bot in degraded
// in-memory mode so a missing optional database cannot crash the process.
func Connect(uri string) error {
	...
	dbName := "musicbot"
	if parsed, parseErr := url.Parse(uri); parseErr == nil && parsed.Path != "" && parsed.Path != "/" {
		dbName = parsed.Path[1:]
	}
	if envName := os.Getenv("MONGO_DB_NAME"); envName != "" {
		dbName = envName
	}
	...
	SudoersDB = db.Collection("sudoers")
	BlockedDB = db.Collection("blocked")
	GBanDB = db.Collection("gban")
	BlacklistedChatsDB = db.Collection("blacklisted_chats")
	ServedChatsDB = db.Collection("served_chats")
	DailyStatsDB = db.Collection("daily_stats")
	CloneOwnerDB = db.Collection("clone_owners")
	CloneBotNameDB = db.Collection("clone_names")
	CloneBotDB = db.Collection("clone_bots")
	CloneCustomDB = db.Collection("clone_custom")
	ChatsDBC = db.Collection("clone_chats")
	UsersDBC = db.Collection("clone_users")
	return nil
}
...
// GetLoop returns the configured loop count for a chat.
...
// SetLoop updates the configured loop count for a chat.
