...
// CreateBtn is the exported form used by plugin packages.
// emojiID is accepted as either a string or numeric custom-emoji ID.
func CreateBtn(markup *tb.ReplyMarkup, text, cb, url string, style int, emojiID interface{}, noEmoji bool) tb.Btn {
	return createBtn(markup, text, cb, url, style, fmt.Sprint(emojiID), noEmoji)
}
